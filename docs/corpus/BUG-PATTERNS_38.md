...
### Dashboard Browser Auto-Open from Subprocess
- **Issue**: Browser doesn't open when dashboard is launched via Python subprocess
- **Root Cause**: `open` command in subprocess context doesn't have proper GUI access
- **Fix**: Triple-fallback approach in `dashboard/open-dashboard.sh`:
  1. Chrome app mode via `nohup open -na`
  2. AppleScript (more reliable from subprocesses)
  3. Default browser via `nohup open`
- **Changes**:
  - Added `nohup` and full stdin/stdout/stderr redirection
  - Added AppleScript fallback for macOS
  - Async browser launch in `start-dashboard.sh` via subshell
- **Files Modified**:
  - `dashboard/open-dashboard.sh` (~10 lines)
  - `dashboard/start-dashboard.sh` (~5 lines)

### lib/atomic.sh: atomic_drain_stdin() Non-TTY Fix
- **Issue**: `atomic_drain_stdin()` function tries to read from `/dev/tty` which fails in non-TTY environments
- **Error**: `/dev/tty: Device not configured`
- **Impact**: Causes task scripts to fail when run via subprocess (UAT mode, Python orchestrators)
- **Fix**: Added `|| true` to line 893:
  ```bash
  while read -t 0.01 -n 1 _discard </dev/tty 2>/dev/null; do :; done || true
  ```
- **Root Cause**: When running bash scripts through Python subprocess.run() without PTY, /dev/tty doesn't exist
- **Pattern**: #1 (Arithmetic with set -e) - even error-suppressed commands need `|| true` in some cases

---

## Recent Fixes (2026-02-04 - Earlier)
...
- **Issue**: Dashboard doesn't automatically pop up during Task 002
  - Running with `&` and `>/dev/null 2>&1` prevented browser from opening
  - UAT mode was trying to open browsers unnecessarily
- **Fix**: Modified `phases/phase00/task002.sh` to:
  1. Skip dashboard launch in UAT mode
  2. Run start-dashboard.sh synchronously (no backgrounding)
  3. Re-open browser if dashboard already running
- **Fix**: Enhanced `dashboard/open-dashboard.sh` with fallbacks:
  - Try Chrome app mode first
  - Fallback to default browser if Chrome unavailable
- **Detection**: Task 002 runs but no browser window opens
- **Prevention**: Don't redirect dashboard scripts to /dev/null
...
### 10. Large-Context LLM Generation Timeouts ⏱️

**Pattern**: Multi-generation workflows with large cumulative context can timeout or fail on later generations.

**Symptoms**:
- Early generations (1-6) complete successfully
- Later generations (7-8) timeout or produce empty output
- Prompt files are very large (>40KB)
- Prior context files are very large (>100KB)
- Error files or temp files left behind (`.err`, `.tmp.*`)

**Example**: Task 205 (PRD Authoring) 8-generation workflow:
- Generations 1-6: ✅ PASS (total ~15,546 words generated)
- Generation 7: ❌ TIMEOUT (prompt: 47KB, context: 114KB)
- Generation 8: ⏳ NOT REACHED

**Root Cause**:
- LLM timeout (default 1200s may be insufficient)
- Context window overflow (even with 200K limit)
- Provider-specific rate limiting
- Cumulative token cost causing throttling

**Solutions**:

**Option 1: Add UAT Bypass** (recommended for testing):
```bash
if [[ "${ATOMIC_UAT_MODE:-false}" == "true" ]]; then
    # Create minimal complete PRD with all 15 sections
    cat > "$prd_file" << 'EOF'
...
**Option 2: Split Large Generations**:
- Generation 7: Split into Gen 7, 8, 9 (one section each)
- Reduces individual prompt size
- More validation points

**Option 3: Context Window Optimization**:
```bash
# Instead of full prior sections, use strategic excerpts
# First 100 + last 50 lines per section
head -100 "$section_file" >> "$context_file"
echo "..." >> "$context_file"
tail -50 "$section_file" >> "$context_file"
```

**Option 4: Increase Timeouts**:
```bash
atomic_invoke "$prompt" "$output" "Gen 7" --timeout=3600  # 1 hour
```

**Option 5: Use Different Provider for Large Gens**:
```bash
# Use Ollama for large-context generations (no CLI issues)
if [[ $gen_num -ge 7 ]]; then
    provider="ollama"
    model="qwen2.5:72b"  # Large context window
fi
```

**Affected Tasks**:
- Task 205 (PRD Authoring) - 8-generation workflow
- Any multi-stage document generation with cumulative context
- Tasks with guardian validation requiring full document review

**Detection**:
- Output file is 0 bytes or empty
- `.err` or `.tmp.*` files present in output directory
- Last successful generation smaller than expected
- Guardian validation never runs for failed generation

**When to Use Each Solution**:
- **UAT bypass**: Testing refactors, state persistence, phase transitions
- **Split generations**: Production use, need full quality
- **Context optimization**: Limited token budget, long documents
- **Increase timeout**: One-off generation, rare failures
- **Different provider**: Consistent large-context needs

**Evidence Files** (Task 205 Gen 7 failure):
```
.outputs/2-prd/prompts/
├── gen-7-prompt.md          # 47KB prompt
├── gen-7-prior.md           # 114KB prior context
├── gen-7-sections.md        # 0 bytes (empty)
├── gen-7-sections.md.err    # Error artifact
└── gen-7-sections.md.tmp.38720  # Temp artifact
```

**Performance Benchmarks** (Task 205):
- Generation 1-3: 30-73s each
- Generation 4: 149s (largest NFR section)
- Generation 5-6: 50-60s each
- Generation 7: 600s+ (timeout)
- Guardian validations: 12-15s each

**Recommendation**: Always add UAT bypass to complex multi-generation tasks, even if production workflow uses full generation. This enables rapid testing of orchestration logic independent of LLM performance.
...
9. **Multi-generation workflows need UAT bypass**: Large-context LLM tasks should have minimal output mode for testing
10. **Always copy lib/ directory early**: Many tasks source lib/atomic.sh - missing lib/ breaks Phase 2+
