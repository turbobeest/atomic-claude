...
task_107_approach_selection() {
    ...
    if [[ "${ATOMIC_UAT_MODE:-false}" == "true" ]]; then
        echo ""
        echo -e "  ${YELLOW}⚡${NC} UAT Mode: Auto-approving first approach"
        echo ""

        # Load first approach if available
        local first_approach=""
        if [[ -f "$approaches_file" ]]; then
            first_approach=$(jq -r '.approaches[0] // {}' "$approaches_file" 2>/dev/null || echo "{}")
        fi

        # Create selected approach
        cat > "$selected_json" << EOF
{
...
EOF
        ...
        cat > "$selected_md" << 'EOF'
# Selected Approach: UAT Test Approach
...
EOF
        ...
        atomic_context_artifact "selected_approach" "$selected_json" "Selected approach (UAT mode)"
        atomic_success "Direction confirmed (UAT mode)"
        return 0
    fi
