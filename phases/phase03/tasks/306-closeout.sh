...
task_306_closeout() {
    ...
    if [[ "${ATOMIC_UAT_MODE:-false}" == "true" ]]; then
        echo ""
        echo -e "  ${YELLOW}⚡${NC} UAT Mode: Auto-approving closeout"
        echo ""

        cat > "$closeout_file" << 'EOF'
# Phase 3: Tasking - Closeout
...
EOF
        ...
        atomic_context_artifact "phase3_closeout" "$closeout_file" "Phase 3 closeout (UAT mode)"
        atomic_success "Phase closeout complete (UAT mode)"
        return 0
    fi
