...
task_302_agent_selection() {
    ...
    if [[ "${ATOMIC_UAT_MODE:-false}" == "true" ]]; then
        echo ""
        echo -e "  ${YELLOW}⚡${NC} UAT Mode: Using core agents only"
        echo ""

        cat > "$agents_file" << 'EOF'
{
  "selected": ["task-decomposer", "dependency-mapper", "work-packager", "task-validator", "coverage-checker"],
  "additional": [],
  "selection_method": "uat_defaults"
}
EOF
        atomic_context_artifact "selected_agents" "$agents_file" "Phase 3 agents (core only, UAT mode)"
        atomic_context_decision "Core agents selected automatically (UAT mode)" "agent-selection"
