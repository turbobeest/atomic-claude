...
task_002_config_collection() {
    ...
    if [[ -n "${SETUP_FILE_PATH:-}" ]] && [[ -f "$SETUP_FILE_PATH" ]]; then
        ...
        # Auto-launch dashboard if not already running (unless in UAT mode)
        if [[ "${ATOMIC_UAT_MODE:-false}" != "true" ]]; then
            if ! lsof -Pi :5174 -sTCP:LISTEN -t >/dev/null 2>&1; then
                atomic_substep "Launching tasks dashboard..."
                if [[ -f "$ATOMIC_ROOT/dashboard/start-dashboard.sh" ]]; then
                    # Run synchronously to ensure browser opens
                    bash "$ATOMIC_ROOT/dashboard/start-dashboard.sh"
                    echo ""
                fi
            else
                atomic_substep "Dashboard already running at http://localhost:5174"
                # Re-open browser if dashboard exists
                if [[ -f "$ATOMIC_ROOT/dashboard/open-dashboard.sh" ]]; then
                    bash "$ATOMIC_ROOT/dashboard/open-dashboard.sh" 2>/dev/null || true
                fi
                echo ""
            fi
        fi
    ...
    # Validate setup.md format before processing
    if ! grep -q -E '^\*\*[^*]+\*\*:' "$SETUP_FILE_PATH" 2>/dev/null; then
        atomic_error "Invalid setup.md format"
        echo ""
        echo -e "${RED}  The setup.md file doesn't contain valid configuration fields.${NC}"
        echo -e "${DIM}  Expected format:${NC}"
        echo ""
        echo -e "  ${DIM}**name**: my-project${NC}"
        echo -e "  ${DIM}**type**: webapp${NC}"
        echo -e "  ${DIM}**description**: Project description${NC}"
        echo ""
        echo -e "${DIM}  File checked: $SETUP_FILE_PATH${NC}"
        echo ""
        return 1
    fi
...
PROMPT_SCHEMA
    ...
    if atomic_invoke "$prompt_file" "$extracted_file" "Extract configuration from setup"; then
        ...
        if grep -q '```json' "$extracted_file"; then
            ...
            # No fences - extract JSON from first { to last }
            # This handles cases where LLM includes explanation text before JSON
            if grep -q '^{' "$extracted_file"; then
                # JSON starts at beginning - file is already clean
                :
            else
                # JSON is embedded in text - extract it
                atomic_substep "Extracting JSON from response..."
                awk '/{/,/^}$/' "$extracted_file" > "${extracted_file}.clean"
                if [[ -s "${extracted_file}.clean" ]]; then
                    mv "${extracted_file}.clean" "$extracted_file"
                fi
            fi
        fi
