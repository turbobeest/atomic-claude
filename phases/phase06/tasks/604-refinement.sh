...
task_604_refinement() {
    ...
    if [[ "${ATOMIC_UAT_MODE:-false}" == "true" ]]; then
        ...
        mkdir -p "$review_dir" "$fixes_dir"

        local refinement_report="$ATOMIC_OUTPUT_DIR/$CURRENT_PHASE/refinement-report.md"
        echo "# Refinement Report (UAT Mode)" > "$refinement_report"
        echo "" >> "$refinement_report"
        echo "All issues addressed (UAT stub)" >> "$refinement_report"

        # Create refinement-report.json for downstream tasks
        jq -n '{
            "refinements": {
