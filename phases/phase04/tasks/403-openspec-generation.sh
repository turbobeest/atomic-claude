...
task_403_openspec_generation() {
    ...
    # UAT Mode: Create minimal specs
    if [[ "${ATOMIC_UAT_MODE:-false}" == "true" ]]; then
        echo ""
        echo -e "  ${YELLOW}⚡${NC} UAT Mode: Creating minimal OpenSpec files"
        echo ""
        mkdir -p "$ATOMIC_ROOT/.claude/specs" "$ATOMIC_OUTPUT_DIR/$CURRENT_PHASE"
        for i in 1 2 3; do
            cat > "$ATOMIC_ROOT/.claude/specs/spec-TASK-00$i.json" << 'EOF'
{
  "test_strategy": "unit-tests",
  "interface_contract": "minimal",
  "mode": "uat"
}
EOF
        done
        echo "{ "specs_generated": 3, "mode": "uat" }" > "$ATOMIC_OUTPUT_DIR/$CURRENT_PHASE/spec-progress.json"
