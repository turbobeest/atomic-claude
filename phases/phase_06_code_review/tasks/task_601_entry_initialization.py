...
def _display_phase_header() -> None:
    ...
    print()
    print_dim("━" * 110)
    print_cyan("  PHASE 06 - CODE REVIEW")
    print_dim("━" * 110)
    print()
    print_bold(print_cyan("""
   ██████╗ ██████╗ ██████╗ ███████╗    ██████╗ ███████╗██╗   ██╗██╗███████╗██╗    ██╗
  ██╔════╝██╔═══██╗██╔══██╗██╔════╝    ██╔══██╗██╔════╝██║   ██║██║██╔════╝██║    ██║
  ██║     ██║   ██║██║  ██║█████╗      ██████╔╝█████╗  ██║   ██║██║█████╗  ██║ █╗ ██║
  ██║     ██║   ██║██║  ██║██╔══╝      ██╔══██╗██╔══╝  ╚██╗ ██╔╝██║██╔══╝  ██║███╗██║
...
def _verify_phase_5(closeout_file: Path) -> bool:
    ...
    print_dim("─" * 100)
    print()
    print_bold("PHASE 5 VERIFICATION")
    print()

    if not closeout_file.exists():
        print_red("✗ Phase 5 closeout not found")
        print()
        print_red("Phase 5 must be completed before starting Phase 6")
        return False
    ...
    try:
        ...
        print_red("✗ Failed to read Phase 5 closeout")
        return False
    ...
    if phase5_status != "complete":
        print_red(f"✗ Phase 5 status: {phase5_status}")
        print()
        print_red("Phase 5 must be complete before starting Phase 6")
        return False

    print_green("✓ Phase 5 complete")
    print_green(f"✓ TDD cycles completed: {tasks_completed} / {total_tasks}")
    print_green(f"✓ Tests passing: {tests_passing} / {total_tests}")
    print_green(f"✓ Unit coverage: {unit_coverage}%")
    print()
...
def _display_code_review_overview() -> None:
    ...
    print_dim("─" * 100)
    print()
    print_bold("CODE REVIEW OVERVIEW")
    print()

    print_dim("Phase 6 performs comprehensive code review across multiple dimensions:")
    print()

    print("  ─" * 50)
    print_bold("CODE REVIEW PROCESS")
    print()
    print_cyan("    Deep Code Review") + "  ───►"
    print("                            ╲")
    print_cyan("    Architecture") + "      ───►  ╲"
    print_cyan("    Compliance") + "               ╲"
    print("                              ────►  " + print_green("Code Refiner"))
    print_cyan("    Performance") + "       ───►  ╱        " + print_green("(Refinement)")
    print_cyan("    Analysis") + "               ╱"
    print("                            ╱")
...
def _display_review_dimensions() -> None:
    ...
    print_dim("─" * 100)
    print()
    print_bold("REVIEW DIMENSIONS")
    print()

    print_cyan("1. Deep Code Review")
    print_dim("   Logic correctness, error handling, edge cases, code clarity")
    print()
    print_cyan("2. Architecture Compliance")
    print_dim("   Pattern adherence, dependency management, separation of concerns")
    print()
    print_cyan("3. Performance Analysis")
    print_dim("   Algorithmic complexity, resource usage, potential bottlenecks")
    print()
    print_cyan("4. Documentation Review")
    print_dim("   Code comments, API docs, README completeness")
    print()
    print_cyan("5. Code Refinement")
    print_dim("   Address findings, apply improvements, maintain test coverage")
    print()
