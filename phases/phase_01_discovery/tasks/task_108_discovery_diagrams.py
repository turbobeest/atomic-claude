...
def execute(atomic_root: Path, output_dir: Path, uat_mode: bool = False) -> bool:
    """
    Execute Task 108: Discovery Diagrams.
    ...
    """
    diagrams_dir = atomic_root / "docs" / "diagrams"
    ...
    if approval == 'n':
        print("  Review diagrams at:", diagrams_dir)
...
def _load_approach_context(output_dir: Path) -> Dict[str, str]:
    ...
    if dialogue_file.exists():
        with open(dialogue_file) as f:
            data = json.load(f)
        vision = data.get("synthesis", {}).get("vision", {})
...
def _generate_diagram(diagram_type: str, output_file: Path, prompts_dir: Path,
    ...
    # Build prompt
    context_section = f"""**Approach:** {context['approach_name']}
