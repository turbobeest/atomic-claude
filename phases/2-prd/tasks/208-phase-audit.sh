...
#   - See: docs/MULTI-AGENT-AUDIT-SPEC.md (to be created)
#
...
_208_legacy_audit() {
    ...
    all_dimensions=$(_208_get_dimension_definitions)
    ...
    case "$profile" in
        minimal)
            dim_count=10
            selected_dims=$(_208_get_dimensions_for_profile "minimal")
            ;;
        standard)
            dim_count=25
            selected_dims=$(_208_get_dimensions_for_profile "standard")
            ;;
        thorough)
            dim_count=40
            selected_dims=$(_208_get_dimensions_for_profile "thorough")
            ;;
        exhaustive)
            dim_count=60
...
_208_get_dimension_definitions() {
    cat << 'DIMENSIONS'
...
_208_get_dimensions_for_profile() {
    local profile="$1"
...
DIMS
        ...
        standard)
            ...
            _208_get_dimension_definitions | grep -E "^PRD-S0[1-5]:|^PRD-R0[1-5]:|^PRD-T0[1-5]:|^PRD-C0[1-5]:|^PRD-K0[1-3]:|^PRD-P0[1-2]:" | head -25
